...
class TestClient {
  ...
  sessionToken = "";
  private ai = new HeuristicAI(cardRegistry, DIFFICULTY_PARAMS.hard, 42);
  ...
  pickAction(): unknown | null {
    if (!this.view || this.seat === null) return null;
    if (playerToActFromView(this.view) !== this.seat) return null;
...
describe("RoomCore（サーバーの権威ロジック）", () => {
  it("2クライアントで終局まで対局できる", () => {
    ...
    for (;;) {
      ...
      if (finished) break;
      const actor = clients.find((c) => c.pickAction() !== null);
  ...
  it("切断してもトークンで復帰でき、最新盤面が届く", () => {
    ...
    // 間違ったトークンでは復帰できない
    const res2 = room.reattach("でたらめ", new TestClient().send);
